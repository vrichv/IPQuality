...
check_bash() {
    ...
    if [ "$current_bash_version" = "0" ] || [ "$current_bash_version" = "1" ] || [ "$current_bash_version" = "2" ] || [ "$current_bash_version" = "3" ]; then
        ...
        echo "bash <(curl -sL https://raw.githubusercontent.com/vrichv/IPQuality/main/ref/upgrade_bash.sh)"
        exit 0
    fi
}
check_bash
...
declare CurlARG=""
declare UA_Browser
declare Media_Cookie=$(curl $CurlARG -s --retry 3 --max-time 10 "https://raw.githubusercontent.com/vrichv/IPQuality/main/ref/cookies.txt")
declare IATA_Database="https://raw.githubusercontent.com/vrichv/IPQuality/main/ref/iata-icao.csv"
shelp_lines=(
    "IP QUALITY CHECK SCRIPT"
    "Usage: bash <(curl -sL https://raw.githubusercontent.com/vrichv/IPQuality/main/ip.sh) [-4] [-6] [-f] [-h] [-i eth0] [-l cn|en|jp|es|de|fr|ru|pt] [-x http://usr:pwd@proxyurl:p]"
    "            -4                             Test IPv4"
    "            -6                             Test IPv6"
...
set_language() {
    ...
    "en" | "jp" | "es" | "de" | "fr" | "ru" | "pt")
        ...
        shead[bash]="bash <(curl -sL https://raw.githubusercontent.com/vrichv/IPQuality/main/ip.sh)"
        shead[git]="https://github.com/vrichv/IPQuality"
...
declare -A browsers=(
    [Chrome]="109.0.0.0 127.0.0.0 128.0.0.0"
    [Firefox]="115.0 113.0 129.0"
    [Edge]="124.0.0.0 125.0.0.0 127.0.0.0")
generate_random_user_agent() {
    ...
    case $browser in
    Chrome)
        UA_Browser="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/$version Safari/537.3"
        ;;
    Firefox)
...
db_maxmind() {
    ...
    maxmind[asn]=$(echo "$RESPONSE" | jq -r '.connection.asn')
    maxmind[org]=$(echo "$RESPONSE" | jq -r '.connection.org')
    maxmind[city]=$(echo "$RESPONSE" | jq -r '.city')
    maxmind[post]=$(echo "$RESPONSE" | jq -r '.postal')
    maxmind[lat]=$(echo "$RESPONSE" | jq -r '.latitude')
    maxmind[lon]=$(echo "$RESPONSE" | jq -r '.longitude')
    #maxmind[rad]=$(echo "$RESPONSE" | jq -r '.null')
    #maxmind[dms]=$(generate_dms "${maxmind[lat]}" "${maxmind[lon]}")
    #maxmind[map]=$(generate_googlemap_url "${maxmind[lat]}" "${maxmind[lon]}" "${maxmind[rad]}")
    maxmind[continentcode]=$(echo "$RESPONSE" | jq -r '.continent_code')
    maxmind[continent]=$(echo "$RESPONSE" | jq -r '.continent')
    maxmind[citycountrycoad]=$(echo "$RESPONSE" | jq -r '.country_code')
    maxmind[citycountry]=$(echo "$RESPONSE" | jq -r '.country')
    maxmind[timezone]=$(echo "$RESPONSE" | jq -r '.timezone.id')
    maxmind[subcode]=$(echo "$RESPONSE" | jq -r '.region_code')
    maxmind[sub]=$(echo "$RESPONSE" | jq -r '.region')
    maxmind[countrycode]=$(echo "$RESPONSE" | jq -r '.country_code')
    maxmind[country]=$(echo "$RESPONSE" | jq -r '.country')
    #maxmind[regcountrycode]=$(echo "$RESPONSE" | jq -r '.country_code')
    #maxmind[regcountry]=$(echo "$RESPONSE" | jq -r '.country')
    if [[ ${maxmind[lat]} != "null" && ${maxmind[lon]} != "null" ]]; then
        maxmind[dms]=$(generate_dms "${maxmind[lat]}" "${maxmind[lon]}")
        maxmind[map]=$(generate_googlemap_url "${maxmind[lat]}" "${maxmind[lon]}" 15)
    else
        maxmind[dms]="null"
        maxmind[map]="null"
    fi
...
check_dnsbl_parallel() {
    ...
    local blacklisted=0
    local other=0
    curl $CurlARG -s "https://raw.githubusercontent.com/vrichv/IPQuality/main/ref/dnsbl.list" | sort -u | xargs -P "$parallel_jobs" -I {} bash -c "result=\$(dig +short \"$reversed_ip.{}\" A); if [[ -z \"\$result\" ]]; then echo 'Clean'; elif [[ \"\$result\" == '127.0.0.2' ]]; then echo 'Blacklisted'; else echo 'Other'; fi" | {
...
show_factor() {
    ...
    echo -ne "\r$Font_Cyan${sfactor[factor]}${Font_I}IP2LOCATION ipapi SCAMALYTICS IPinfo IPWHOIS ipregistry IPQS ipdata$Font_Suffix\n"
    tmp_factor=$(format_factor "${ip2location[countrycode]}" "${ipapi[countrycode]}" "${scamalytics[countrycode]}" "${ipinfo[countrycode]}" "${ipwhois[countrycode]}" "${ipregistry[countrycode]}" "${ipqs[countrycode]}" "${ipdata[countrycode]}")
    echo -ne "\r$Font_Cyan${sfactor[countrycode]}$Font_Suffix$tmp_factor\n"
    tmp_factor=$(format_factor "${ip2location[proxy]}" "${ipapi[proxy]}" "${scamalytics[proxy]}" "${ipinfo[proxy]}" "${ipwhois[proxy]}" "${ipregistry[proxy]}" "${ipqs[proxy]}" "${ipdata[proxy]}")
    echo -ne "\r$Font_Cyan${sfactor[proxy]}$Font_Suffix$tmp_factor\n"
    tmp_factor=$(format_factor "${ip2location[tor]}" "${ipapi[tor]}" "${scamalytics[tor]}" "${ipinfo[tor]}" "${ipwhois[tor]}" "${ipregistry[tor]}" "${ipqs[tor]}" "${ipdata[tor]}")
    echo -ne "\r$Font_Cyan${sfactor[tor]}$Font_Suffix$tmp_factor\n"
    tmp_factor=$(format_factor "${ip2location[vpn]}" "${ipapi[vpn]}" "${scamalytics[vpn]}" "${ipinfo[vpn]}" "${ipwhois[vpn]}" "${ipregistry[vpn]}" "${ipqs[vpn]}" "${ipdata[vpn]}")
    echo -ne "\r$Font_Cyan${sfactor[vpn]}$Font_Suffix$tmp_factor\n"
    tmp_factor=$(format_factor "${ip2location[server]}" "${ipapi[server]}" "${scamalytics[server]}" "${ipinfo[server]}" "${ipwhois[server]}" "${ipregistry[server]}" "${ipqs[server]}" "${ipdata[server]}")
    echo -ne "\r$Font_Cyan${sfactor[server]}$Font_Suffix$tmp_factor\n"
    tmp_factor=$(format_factor "${ip2location[abuser]}" "${ipapi[abuser]}" "${scamalytics[abuser]}" "${ipinfo[abuser]}" "${ipwhois[abuser]}" "${ipregistry[abuser]}" "${ipqs[abuser]}" "${ipdata[abuser]}")
    echo -ne "\r$Font_Cyan${sfactor[abuser]}$Font_Suffix$tmp_factor\n"
    tmp_factor=$(format_factor "${ip2location[robot]}" "${ipapi[robot]}" "${scamalytics[robot]}" "${ipinfo[robot]}" "${ipwhois[robot]}" "${ipregistry[robot]}" "${ipqs[robot]}" "${ipdata[robot]}")
...
get_opts() {
    while getopts "i:l:x:fh46" opt; do
        ...
        x)
            xproxy="$OPTARG"
            if [[ -z $xproxy ]] || ! curl -sL -x "$xproxy" --connect-timeout 5 --max-time 10 https://ip.sb -o /dev/null; then
                ERRORcode=8
            else
                usePROXY=" -x $xproxy"
                CurlARG="$usePROXY"
